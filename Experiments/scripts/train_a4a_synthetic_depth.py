"""
Eksperimen A.4a: Synthetic Depth Only - V2
Train YOLOv11n pada dataset Synthetic Depth (Depth-Anything-V2) dengan uniform geometric augmentation
...
Special: Reset BatchNorm stats untuk adaptasi synthetic depth data

Seeds: 42, 123, 456, 789, 101 (5 seeds)
Output: kaggleoutput/a4a_depth_results.txt
...
def get_config():
    ...
    if os.path.exists('/kaggle/working'):
        ...
        return {
            ...
            'data_yaml': '/kaggle/working/dataset/ffb_localization_depth_synthetic.yaml',
            'dataset_path': '/kaggle/working/dataset/ffb_localization_depth_synthetic',
        ...
        return {
            'base_dir': base,
            'data_yaml': str(base / 'configs' / 'ffb_localization_depth_synthetic.yaml'),
            'dataset_path': str(base / 'datasets' / 'ffb_localization_depth_synthetic'),
...
def train_with_seed(seed, config, output_file):
    ...
    train_config['name'] = f'exp_a4a_synthetic_depth_seed_{seed}'
    ...
    try:
        ...
        # Reset BatchNorm stats for synthetic depth adaptation
...
def main():
    ...
    print("="*70)
    print("Eksperimen A.4a: Synthetic Depth Only - V2")
    ...
    # Open results file
    results_path = config['output_dir'] / 'a4a_depth_results.txt'
    print(f"\nResults will be saved to: {results_path}")

    with open(results_path, 'w') as output_file:
        # Write header
        output_file.write("="*70 + "\n")
        output_file.write("Eksperimen A.4a: Synthetic Depth Only - V2\n")
    ...
    print("\nModel locations:")
    for seed in SEEDS:
        print(f"  Seed {seed}: runs/detect/exp_a4a_synthetic_depth_seed_{seed}/weights/best.pt")

    print("\nComparison with A.2 (Real Depth):")
    print("  - A.2: Real depth from RealSense sensor")
    print("  - A.4a: Synthetic depth from Depth-Anything-V2")
