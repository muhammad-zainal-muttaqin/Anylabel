"""
Eksperimen A.4b: RGB + Synthetic Depth (4-channel) - V2
Train YOLOv11n dengan RGB + Synthetic Depth 4-channel input dan uniform geometric augmentation
...
Special:
    - Reset BatchNorm stats untuk adaptasi RGBD data
    - Modify first conv layer dari 3ch ke 4ch

Seeds: 42, 123, 456, 789, 101 (5 seeds)
Output: kaggleoutput/a4b_rgbd_results.txt
...
def get_config():
    ...
    if os.path.exists('/kaggle/working'):
        ...
        return {
            ...
            'data_yaml': '/kaggle/working/dataset/ffb_localization_rgbd_synthetic.yaml',
            'dataset_path': '/kaggle/working/dataset/ffb_localization',
            'depth_dir': '/kaggle/working/dataset/depth_synthetic_da2',
        ...
        return {
            'base_dir': base,
            'data_yaml': str(base / 'configs' / 'ffb_localization_rgbd_synthetic.yaml'),
            'dataset_path': str(base / 'datasets' / 'ffb_localization'),
            'depth_dir': str(base / 'datasets' / 'depth_synthetic_da2'),
...
def modify_model_for_4ch(base_model='yolo11n.pt'):
    ...
    with torch.no_grad():
        ...
        # Initialize depth channel from R channel
    ...
    # Save modified model
    modified_path = Path(r'D:\Work\Assisten Dosen\Anylabel\Experiments') / 'yolo11n_4ch.pt'
...
def create_rgbd_config(config):
    """Create YAML config for RGBD synthetic dataset."""
    yaml_content = f"""# Dataset Configuration for A.4b - RGB + Synthetic Depth (4-channel)
path: {config['dataset_path'].replace('\\', '/')}
depth_dir: {config['depth_dir'].replace('\\', '/')}
...
def verify_dataset(config):
    ...
    if missing:
        print(f"ERROR: Missing RGB directories:")
        for d in missing:
            print(f"  - {d}")
        return False
    ...
    if not depth_dir.exists():
        print(f"ERROR: Synthetic depth directory not found: {depth_dir}")
        return False

    # Count files
    train_images = list((dataset_path / 'images' / 'train').glob('*.png'))
    depth_images = list(depth_dir.glob('*.png'))

    print(f"Dataset verified:")
    print(f"  - RGB train images: {len(train_images)}")
    print(f"  - Synthetic depth images: {len(depth_images)}")

    return len(train_images) > 0 and len(depth_images) > 0
...
def train_with_seed(seed, config, output_file):
    ...
    train_config['name'] = f'exp_a4b_rgbd_synthetic_seed_{seed}'
    ...
    try:
        ...
        # Reset BatchNorm stats for RGBD adaptation
...
def main():
    ...
    print("="*70)
    print("Eksperimen A.4b: RGB + Synthetic Depth (4-channel) - V2")
    ...
    # Create RGBD config (only for local)
    if not config['is_kaggle']:
        create_rgbd_config(config)

    # Verify dataset
    print("\nVerifying dataset...")
    if not verify_dataset(config):
        print("ERROR: Dataset verification failed!")
        print("\nPlease ensure synthetic depth dataset is prepared:")
        print("  1. python generate_synthetic_depth.py")
        print("  2. python prepare_synthetic_depth_data.py")
        sys.exit(1)

    # Modify model for 4-channel input
    print("\nPreparing 4-channel model...")
    modified_model_path = modify_model_for_4ch('yolo11n.pt')
    if modified_model_path is None:
        print("ERROR: Failed to create 4-channel model!")
        sys.exit(1)

    # Open results file
    results_path = config['output_dir'] / 'a4b_rgbd_results.txt'
    print(f"\nResults will be saved to: {results_path}")

    with open(results_path, 'w') as output_file:
        # Write header
        output_file.write("="*70 + "\n")
        output_file.write("Eksperimen A.4b: RGB + Synthetic Depth (4-channel) - V2\n")
    ...
    print("\nModel locations:")
    for seed in SEEDS:
        print(f"  Seed {seed}: runs/detect/exp_a4b_rgbd_synthetic_seed_{seed}/weights/best.pt")

    print("\nComparison:")
    print("  - A.3: RGB + Real Depth")
    print("  - A.4b: RGB + Synthetic Depth")
    print("  - Expected: A.3 > A.4b (if synthetic depth quality is good)")
