# FFB Oil Palm Detection - 5 Seed V2 Results (Revisi)

**Generated:** 2026-01-29 | **Seeds:** 42, 123, 456, 789, 101 | **Model:** YOLOv11n, 100 epochs
**Revisi:** Uniform augmentation + BN Reset untuk depth experiments
...
| Finding | Details |
|---------|---------|
| **Best Localization** | A.1 (RGB Only) - **0.8385 mAP50** |
...
| **Late Fusion (A.5)** | **0.6742 mAP50** - Below baseline |
...
**Note:** Semua eksperimen menggunakan **uniform augmentation**: translate=0.1, scale=0.5, fliplr=0.5, HSV/mosaic/mixup disabled.
...
**Mean±Std:** 0.7325±0.0419 | **BN Reset:** Menggunakan 100 gambar training asli
...
**Mean±Std:** 0.8403±0.0161 | **BN Reset:** Callback on_train_start dengan 100 real images
...
<details>
<summary><b>A.5 - Late Fusion (V2) - 5 Epochs Only</b></summary>
...
**Mean±Std:** 0.6742±0.0887 | **Note:** Training 100 epochs sedang berjalan
...
### Perbandingan dengan V1 (Sebelum Revisi)
...
1. **RGBD Fusion Improvement:** A.3 sekarang hampir setara dengan RGB baseline (selisih hanya 0.2%)
2. **BN Reset Effectiveness:** A.4b meningkat 1.2% dengan BN reset menggunakan real images
3. **Uniform Augmentation Impact:** Penurunan performa A.1 karena HSV augmentation dinonaktifkan
4. **Real > Synthetic Consistent:** Real depth (0.733) masih unggul dari synthetic (0.653) sebesar 7.9%
5. **Late Fusion:** Masih dalam training (100 epochs), hasil sementara 5 epochs menunjukkan potensi
...
| A.5 Late Fusion | 0.0887 | High Variance (5 epochs) |
...
| **Simplest Setup** | A.1 RGB Only | Hampir setara A.3, lebih sederhana |
| **No Depth Sensor** | A.4b RGB+Synthetic | Hanya 1.7% di bawah RGB, tanpa sensor |
| **Depth Only** | A.2 Real Depth | 0.7325 mAP50 dengan BN reset |
| **Fusion Exploration** | A.5 Late Fusion | Menunggu hasil 100 epochs |
...
| A.5 | train_a5_late_fusion | ⏳ Training 100 epochs |
